  ...
  module ClassMethods
    def auto_session_timeout(seconds = nil, sign_in_path = nil)
      ...
      prepend_before_action do |c|
        sign_out = if sign_in_path && request.fullpath == sign_in_path && request.method_symbol == :post
                         false
                       else
                         true
                       end

        if c.session[:auto_session_expires_at] && c.session[:auto_session_expires_at] < Time.current && sign_out
          ...
          unless c.request.original_url.start_with?(c.send(:active_url))
            ...
            c.session[:auto_session_expires_at] = Time.current + offset if offset && offset > 0
          end
        end
      end
    end
